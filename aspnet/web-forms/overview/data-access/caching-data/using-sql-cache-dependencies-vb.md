...
**图 2**：将 `NORTHWND.MDF` 数据库从文件夹中 `App_Data` ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image2.png)) 
...
[![创建名为 ProductsDataSourceDeclarative 的新 ObjectDataSource](using-sql-cache-dependencies-vb/_static/image5.gif)](using-sql-cache-dependencies-vb/_static/image3.png)

**图 5**：创建名为 (的新 ObjectDataSource `ProductsDataSourceDeclarative` [单击以查看完全大小的图像](using-sql-cache-dependencies-vb/_static/image4.png)) 
...
**图 6**：将 UpdateProduct 重载用于三个输入参数 ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image6.png)) 
...
**图 7**：将 "插入" 和 "删除" 选项卡的下拉列表设置为 "无 () " ([单击以查看完全大小的图像](using-sql-cache-dependencies-vb/_static/image8.png)) 
...
[![每次对 GridView 进行分页、编辑或排序时，都将激发选择事件的 ObjectDataSource](using-sql-cache-dependencies-vb/_static/image8.gif)](using-sql-cache-dependencies-vb/_static/image9.png)

**图 8**： `Selecting` 每次对 GridView 进行分页、编辑或排序 ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image10.png)) 

正如我们在 [使用 ObjectDataSource 缓存数据](caching-data-with-the-objectdatasource-vb.md) 中看到的那样，将 `EnableCaching` 属性设置为 `True` 会导致 ObjectDataSource 在其属性指定的持续时间内缓存其数据 `CacheDuration` 。 ObjectDataSource 还具有一个属性，该[ `SqlCacheDependency` 属性](https://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource.sqlcachedependency.aspx)使用模式将一个或多个 SQL 缓存依赖项添加到缓存的数据中：
...
[![使用产品和类别的 SQL 缓存依赖关系配置 ObjectDataSource 以支持缓存](using-sql-cache-dependencies-vb/_static/image9.gif)](using-sql-cache-dependencies-vb/_static/image11.png)

**图 9**：配置 ObjectDataSource 以支持使用上的 SQL 缓存依赖关系的缓存 `Products` ， `Categories` ([单击查看完全大小的图像](using-sql-cache-dependencies-vb/_static/image12.png)) 
...
**图 10**：修改 Products 表逐出缓存的产品数据 ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image14.png)) 

## <a name="step-6-programmatically-working-with-thesqlcachedependencyclass"></a>步骤6：以编程方式使用 `SqlCacheDependency` 类

[体系结构教程中的缓存数据](caching-data-in-the-architecture-vb.md)查看了在体系结构中使用单独的缓存层的好处，而不是将缓存与 ObjectDataSource 紧密耦合起来。 在本教程中，我们创建了一个 `ProductsCL` 类，用于演示如何以编程方式使用数据缓存。 若要在缓存层中使用 SQL 缓存依赖项，请使用 `SqlCacheDependency` 类。
...
**图 11**：配置 ObjectDataSource 以使用 `ProductsCL` 类 ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image16.png)) 
...
**图 12**： `GetProducts` 从 "选择选项卡 s" 下拉列表中选择方法 ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image18.png)) 
...
**图 13**：从 "更新" 选项卡 s 下拉列表中选择 "UpdateProduct" 方法 ([单击查看全尺寸图像](using-sql-cache-dependencies-vb/_static/image20.png)) 
...
> [!NOTE]
> 当通过中的 GridView 编辑某个产品时，可能会出现这种延迟 `SqlCacheDependencies.aspx` 。 在 [体系结构中的缓存数据](caching-data-in-the-architecture-vb.md) 教程中，我们添加了 `MasterCacheKeyArray` 缓存依赖关系，以确保通过类方法进行编辑的数据 `ProductsCL` `UpdateProduct` 已从缓存中逐出。 但是，在此步骤中先前修改方法时，我们将替换此缓存依赖关系，因此，在 `AddCacheItem` `ProductsCL` 轮询系统记录对表的更改之前，类将继续显示缓存的数据 `Products` 。 本文介绍如何 `MasterCacheKeyArray` 在步骤7中重新引入缓存依赖项。
