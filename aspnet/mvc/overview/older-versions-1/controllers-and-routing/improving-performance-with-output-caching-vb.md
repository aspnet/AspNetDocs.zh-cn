...
请注意，清单&lt;4&gt;中的 OutputCache 属性包括一个位置属性，该属性设置为值输出缓存位置.客户端。 OutputCache&lt;&gt;属性还包括 NoStore 属性。 NoStore 属性用于通知代理服务器和浏览器它们不应存储缓存内容的永久副本。
...
清单7中的控制器说明了如何使用&lt;OutputCache&gt;属性将 Cache1Hour 配置文件应用于控制器操作。
...
输出缓存为您提供了一种非常简单的方法来显著提高ASP.NET MVC 应用程序的性能。 在本教程中，您学习了如何使用&lt;OutputCache&gt;属性缓存控制器操作的输出。 您还学习了如何修改&lt;OutputCache&gt;属性的属性，如持续时间和 VaryByParam 属性，以修改内容的缓存方式。 最后，您学习了如何在 Web 配置文件中定义缓存配置文件。
